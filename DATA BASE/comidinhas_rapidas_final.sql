...
CREATE TABLE IF NOT EXISTS `comidinhas_rapidas`.`pedido` (
  ...
  INDEX `fk_pedido_pagamento_idx` (`pagamento_id_pagamento` ASC),
  INDEX `fk_pedido_mesa_idx` (`mesa_id_mesa` ASC),
...
CREATE TABLE IF NOT EXISTS `comidinhas_rapidas`.`pagamento_cliente` (
  ...
  INDEX `fk_pagamento_has_cliente_cliente_idx` (`cliente_id_cliente` ASC),
  INDEX `fk_pagamento_has_cliente_pagamento_idx` (`pagamento_id_pagamento` ASC),
  PRIMARY KEY (`id_pagamento_cliente`),
  CONSTRAINT `fk_pagamento_has_cliente_pagamento`
    FOREIGN KEY (`pagamento_id_pagamento`)
    REFERENCES `comidinhas_rapidas`.`pagamento` (`id_pagamento`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_pagamento_has_cliente_cliente`
...
CREATE TABLE IF NOT EXISTS `comidinhas_rapidas`.`pedido_produto` (
  ...
  INDEX `fk_pedido_has_produto_produto_idx` (`produto_id_produto` ASC),
